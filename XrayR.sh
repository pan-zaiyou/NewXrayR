...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/pan-zaiyou/NewXrayR/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/pan-zaiyou/NewXrayR/main/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
